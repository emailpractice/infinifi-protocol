...
contract Accounting is CoreControlled {
    ...
    function setOracle(
        address _asset,
        address _oracle
    ) external onlyCoreRole(CoreRoles.ORACLE_MANAGER) {
    ...
    function totalAssetsValue() external view returns (uint256 _totalValue) {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            uint256 _assets = _calculateTotalAssets(
                FarmRegistry(farmRegistry).getAssetFarms(assets[i])
            ); //上面的Ioracle有明確interface 但這邊的FarmRegistry沒有，這是可以的。因為solidity合約本身
    ...
    function totalAssetsValueOf(
        uint256 _type
    ) external view returns (uint256 _totalValue) {
        address[] memory assets = FarmRegistry(farmRegistry).getEnabledAssets();
        for (uint256 i = 0; i < assets.length; i++) {
            uint256 assetPrice = IOracle(oracle[assets[i]]).price();
            address[] memory assetFarms = FarmRegistry(farmRegistry)
                .getAssetTypeFarms(assets[i], uint256(_type));
    ...
    function totalAssets(address _asset) external view returns (uint256) {
        return
            _calculateTotalAssets(
                FarmRegistry(farmRegistry).getAssetFarms(_asset)
            );
    }
    ...
    function totalAssetsOf(
        ...
        return
            _calculateTotalAssets(
                FarmRegistry(farmRegistry).getAssetTypeFarms(
                    _asset,
                    uint256(_type)
                )
            );

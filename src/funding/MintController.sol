...
contract MintController is Farm, IMintController {
        ...
        returns (uint256)
    {
        // checks
        require(
            _assetAmountIn >= minMintAmount,
            MintAmountTooLow(_assetAmountIn, minMintAmount)
        );
        ...
        // @ seashell msg.sender是infiniFiGatewayV1.sol 那邊的mint先收使用者錢，然後這個合約再從他那邊拿錢
        ERC20(assetToken).safeTransferFrom(
            msg.sender,
            address(this),
            _assetAmountIn
        );
        ...
        if (_afterMintHook != address(0)) {
            IAfterMintHook(_afterMintHook).afterMint(
                _to,
                _assetAmountIn,
                receiptAmountOut
            );
